module CalendarHelper
  CALENDAR_PARTIAL = {  "WidgetTodo" =>     {:calendar => { :path => "/widgets/todo/", :filename => "calendar"}, :panel => { :path => "/widgets/todo/", :filename => "calendar_panel"}, :activity_list => { :path => "/widgets/todo/", :filename => "activity_list_item"}},
	...
	CALENDAR = [WidgetTodo, CalendarEntry, CalendarTodo] unless const_defined?(:CALENDAR)
  ...
  def activities start=nil, stop=nil, options = { :reload => false, :cache => true }
    ...
    # Create a array of all activities to be included in the calendar
	  activities = CALENDAR.collect{ |c| c.get_for_calendar(current_user.id, to_date(current_user.adjusted_time(start)), to_date(current_user.adjusted_time(stop)), :build_recurring => true) }
  ...
  def generate_calendar view = :month, date = nil
    case view.to_sym
    when :day then generate_day_calendar(date)
    when :week then generate_month_calendar
    when :year then generate_year_calendar
    else generate_month_calendar
    end
  end
