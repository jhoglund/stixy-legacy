...
ENV['TMPDIR'] ||= '/tmp'
ENV['LOGS'] ||= '/data/stixy/shared/log'
ENV['TRAC'] ||= '/home/stixy/trac/stixy'
ENV['REPO'] ||= '/home/stixy/svn'
ENV['RESTORE'] ||= '/home/stixy/backups'
...
namespace :s3 do
    ...
    # Cron job
    # 40 * * * * cd /home/deploy/current && rake s3:backup_hourly RAILS_ENV=production > /dev/null &
    
    desc "Backup everything to S3"
    # "s3:backup:db",  - start db backups to hourly when its smaller then 600 mbs
    task :backup_hourly => ["s3:backup:db", "s3:manage:clean_up", "s3:manage:list"]
    # task :backup_daily => ["s3:backup:scm", "s3:backup:logs", "s3:backup:trac", "s3:manage:clean_up", "s3:manage:list"]
    task :backup_daily => ["s3:backup:logs", "s3:manage:clean_up", "s3:manage:list"]
  ...
  namespace :manage do
      ...
      task :clean_up  do
          ...
          cleanup_bucket('db', keep_num * 24)  # cause its an hourly, and we stil want 10 days of backups
...
private
  ...
  def mysqldump(archive, database_override = nil)
	...
	config_file[RAILS_ENV]['host']
      ...
      database, user, password, host = db_info
