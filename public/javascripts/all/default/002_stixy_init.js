...
Stixy.ua.unless = function (regExp) {
	if (navigator.userAgent.match(regExp)) {
		var json = Stixy.JSON.decode(unescape('<%= CGI.escape({ :replace => { :popup_content => render( :partial => "/shared/popup/non_supported_feature").gsub(/\n/,"") } }.to_json) %>'))
		Stixy.extend(json.replace, Stixy.JSON.decode(unescape('<%= CGI.escape({ :popup_buttons => render( :partial => "/shared/popup/close_button").gsub(/\n/,"") }.to_json) %>')))
